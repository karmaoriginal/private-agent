...
- Check the "PREVIOUS ACTION RESULT" before acting: if it already shows `Typed "..."` with the text you need, do NOT type it again — move on (e.g. press_enter or click submit). Retyping the same text is almost never correct.
...
''';
  ...
  Future<String> executeTask(String userGoal) async {
    ...
    for (int step = 0; step < _aiService.maxSteps; step++) {
      ...
      // Nudge the model to wrap up before it runs out of steps, instead of
      // silently getting cut off by the max-steps limit. This is what lets
      // a short step limit still end with a useful "done" instead of the
      // generic "reached maximum steps" failure.
      String stepUrgencyHint = '';
      if (!_aiService.disableMaxSteps) {
        final stepsRemaining = _aiService.maxSteps - (step + 1);
        if (stepsRemaining <= 2) {
          stepUrgencyHint =
              '\n\nIMPORTANT: Only $stepsRemaining step(s) left before this task is force-stopped. '
              'Wrap up NOW — either finish the goal and respond with action "done", or if it genuinely '
              'cannot be finished in time, respond with action "done" anyway and explain in "reasoning" '
              'what you managed to do, so the user gets useful feedback instead of nothing.';
        }
      }
...
$screenContent$prevResultStr$failureHint$stepUrgencyHint
Step ${step + 1}/${_aiService.maxSteps}. Look at the text dump and coordinates. What is the next action?''';
      ...
      try {
        ...
        final aiFuture = _aiService.sendTaskMessage(
          _taskSystemPrompt,
          prompt,
          // Surface rate-limit/network retries as progress messages instead
          // of leaving the user staring at a stuck step with no feedback.
          onRetry: (msg) => _report(msg),
        );
      ...
      // Track repeats by action + exact parameters, not just the action
      // name. Comparing action name alone let the model type the same text
      // (or click the same element) over and over without ever being
      // blocked, since type_text/click_text had no repeat limit — this is
      // what caused things like a Termux command getting typed several
      // times in a row when the screen dump didn't visibly reflect it yet.
      final actionSignature = '$action:${jsonEncode(params)}';
      sameActionCount = actionSignature == lastActionSignature
          ? sameActionCount + 1
          : 1;
      ...
      if (sameActionCount > repeatLimit) {
        final blockedResult =
            'Blocked repeated $action action with identical parameters. '
            'That was already done — check the current screen before repeating it, '
            'or move on to a different action.';
