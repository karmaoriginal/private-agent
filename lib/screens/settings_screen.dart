...
class _SettingsScreenState extends State<SettingsScreen>
  ...
  late TextEditingController _customSystemPromptController;
  ...
  void initState() {
    ...
    _customSystemPromptController = TextEditingController(
      text: widget.aiService.customSystemPrompt,
    );
    ...
    _customSystemPromptController.addListener(_autoSave);
  ...
  void dispose() {
    ...
    _customSystemPromptController.removeListener(_autoSave);
  ...
  void _autoSave() {
    ...
    widget.aiService.saveCustomSystemPrompt(_customSystemPromptController.text);
              ...
              Wrap(
                ...
                children: [
                  ...
                  ActionChip(
                    ...
                    tooltip: 'Standard OpenAI API (paid, api.openai.com)',
                    onPressed: () {
                      _baseUrlController.text = AiService.openaiBaseUrl;
                      _modelController.text = AiService.openaiDefaultModel;
                    },
                  ),
                  ActionChip(
                    label: const Text('Gemini', style: TextStyle(fontSize: 11)),
                    tooltip: "Google Gemini's OpenAI-compatible endpoint",
                    onPressed: () {
                  ...
                  ActionChip(
                    ...
                    tooltip:
                        'Anthropic Messages API — needs a paid API key from '
                        'console.anthropic.com. NOT the same as a Claude.ai '
                        'subscription (Pro/Max), which has no API access.',
                    onPressed: () {
                      _baseUrlController.text = AiService.anthropicBaseUrl;
                      _modelController.text = AiService.anthropicDefaultModel;
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Claude needs a paid API key from console.anthropic.com '
                            '— your Claude.ai app subscription will not work here.',
                          ),
            ...
            children: [
              ...
              if (_useSystemPrompt) ...[
                ...
                TextField(
                  ...
                  minLines: 2,
                  decoration: _buildInputDecoration(
                    labelText: 'Additional System Prompt (optional)',
                    hintText:
                        'Extra instructions appended after the built-in prompt, '
                        'e.g. "Always reply in Spanish" or "Prefer Firefox over Chrome"',
                    prefixIcon: const Icon(Icons.edit_note_rounded, size: 18),
                  ),
                ),
                const SizedBox(height: 4),
                Text(
                  'Layered on top of the built-in prompt for both chat and multi-step tasks — it does not replace it.',
                  style: TextStyle(
                    fontSize: 11,
